...
def write_to_file(xs, phi, uncol, tfinal, source_name, npnts, x0_or_sigma, c):
    data_folder = Path("benchmarks")
    bench_file_path = data_folder / 'benchmarks.hdf5'
    bench_file_path2 = data_folder / 'benchmarks_conditions.hdf5'
    ...
    if source_name != 'plane_IC':
        ...
        f.close()
    with h5py.File(bench_file_path2,'r+') as f:
        if f.__contains__(source_name + f'/t = {tfinal}'):
            del f[source_name + f'/t = {tfinal}'] 
        f.create_dataset(source_name + f'/t = {tfinal}', (3, 1), dtype = "f", data=(tfinal, x0_or_sigma, c))
    f.close()
